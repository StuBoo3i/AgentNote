...
## 2026-05-17 23:10 CST 追加记录：runner 透传长表 Final Evidence 配置

### 为什么修改

新增的 3 个 Final Evidence 长表配置如果不在 runner 里继续透传，benchmark 和 official task 跑起来仍然会使用 agent 代码默认值，配置文件无法真正控制行为。

### 修改成了什么逻辑

`_run_single_task_core()` 创建 `LangGraphAgentConfig` 时继续新增透传：

```text
final_evidence_materialize_long_tables
final_evidence_long_table_min_rows
final_evidence_block_mismatched_long_table
```

### 影响

只要走 `langgraph` 框架，单任务运行、benchmark 和 official eval 都会统一使用这套长表可信落地策略。

## 2026-05-18 15:23 CST 追加记录：删除 bootstrap prompt 配置透传

### 为什么修改

`bootstrap_observations` 既然已经从 agent 主链路删掉，runner 继续透传 `prompt_max_bootstrap_observations` 只会保留一条无效参数链。

### 修改成了什么逻辑

`_run_single_task_core()` 创建 `LangGraphAgentConfig` 时删除了：
...
runner 到 agent 的配置面收窄为真实生效字段，避免后续误以为 bootstrap 仍是现有 prompt 机制的一部分。

## 2026-05-18 15:27 CST 追加记录：删除失效的 planning/context-pack budget 透传

### 为什么修改

`planning_context_char_budget` 和 `context_pack_char_budget` 已经不再被 agent 使用，runner 继续透传这两个字段没有意义。
...
runner 和 agent 的参数面继续保持一致，避免配置还在传、运行时却已无效的隐性死字段。
