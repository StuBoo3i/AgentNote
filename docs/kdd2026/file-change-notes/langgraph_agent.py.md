...
## 2026-05-17 17:16 CST 追加：阻断未授权 grouped aggregate

### 修改内容

新增了一个很窄的 validation 条件：

```python
def _sql_uses_group_aggregate(sql: str) -> bool:
    return bool(
        re.search(r"\\bgroup\\s+by\\b", sql, flags=re.IGNORECASE)
        and re.search(r"\\b(?:sum|avg|count)\\s*\\(", sql, flags=re.IGNORECASE)
    )
```

然后在 `_context_pack_source_errors()` 里读取 `answer_contract.aggregation_policy`。

如果该策略是：

```text
disallow_implicit_group_aggregate
```

并且最终 SQL 真的用了 `GROUP BY + SUM/AVG/COUNT`，就返回 recoverable blocker，提示改成 row-level metric ranking。

### 影响

这只拦截 grouped aggregate，不会拦截 row-level `MIN(cost)` 子查询，也不会碰其他显式聚合题。

## 2026-05-17 23:10 CST 追加记录：接入 Final Evidence 长表策略

### 为什么修改

`LangGraphReActAgent` 是 Final Evidence 生效的真实入口。只有把新的长表物化和 mismatch 阻断参数从 config 传入 answer 对齐/校验阶段，这次机制增强才会在运行时生效。
...
- `align_answer_with_final_evidence()`：控制是否直接用高置信 evidence projection 物化长表。
- `validate_answer_with_final_evidence()`：控制长表 mismatch 是否作为 fatal error。
...
```text
有没有高置信、完整、未截断的 projectable long-table evidence
```

如果有，最终答案直接来自 evidence projection；如果没有，但发现模型手工复制出的长表和最近完整 evidence 明显不一致，就在 validation 阶段失败，不再写出错误 CSV。

## 2026-05-18 15:23 CST 追加记录：移除并行上下文，只保留 Task Context Pack

### 为什么修改

`LangGraphReActAgent` 原来在 `build_plan` 和执行阶段同时注入：
...
这会造成上下文重复、优先级不清，也让 `Task Context Pack` 的 primary planning context 定位失效。

### 修改成了什么逻辑

`profile_context` 节点现在只做三件事：

- `list_context` 获取文件树
- inspect 选中的文件，形成 `source_summaries`
- 构建 unifiedDB profile 后填充 `Task Context Pack`

删除了：

- graph state 中的 `context_profile`
- `context_summary`
- `task_snapshot`
- `bootstrap_observations`
- `_load_task_snapshot()`
- `_build_context_summary()`
- `_select_bootstrap_observations()`

`build_plan` 现在只接收 `Task Context Pack` 作为规划上下文。执行阶段 prompt 也只保留：
...
现在整条 LangGraph 主路只围绕一个上下文载体运转，避免模型在原始摘要层和解释层之间来回摇摆。

## 2026-05-18 15:27 CST 追加记录：取消 Task Context Pack 独立截断

### 为什么修改

`Task Context Pack` 已经是唯一上下文载体，继续对它应用 `context_pack_char_budget` 截断，会直接损失唯一上下文里的 schema/source/contract 信息。

### 修改成了什么逻辑

`_build_plan_messages()` 和 `_build_messages()` 不再通过：

```text
_render_json_for_prompt(..., max_chars=context_pack_char_budget)
```

渲染 `Task Context Pack`，改为直接完整 `json.dumps(..., indent=2)` 注入 prompt。

### 作用

现在 pack 不会在进入 planning 或 execution prompt 前被单独裁掉；剩余的总量控制只由整段 prompt 的总预算和历史压缩承担。

## 2026-05-18 15:41 CST 追加记录：取消 high_level_plan 独立截断

### 为什么修改

`high_level_plan` 仍然在 execution prompt 里作为执行约束使用，但之前还在经过：

```text
_render_json_for_prompt(..., max_chars=execution_context_char_budget // 2)
```

这会让 plan 本身被局部摘要，导致执行阶段看到的规划信息不完整。

### 修改成了什么逻辑

`_build_messages()` 不再用 `_render_json_for_prompt()` 渲染 `high_level_plan`，改为直接：

```text
json.dumps(state["high_level_plan"], ensure_ascii=False, indent=2)
```
